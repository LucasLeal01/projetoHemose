...
function PacientesPage() {
  ...
  useEffect(() => {
    const mockPacientes: Paciente[] = [
      { id: 1, nome: 'João Silva', leito: '101A', status: 'Estável', diagnostico: 'Pneumonia' },
      { id: 2, nome: 'Maria Souza', leito: '102B', status: 'Crítico', diagnostico: 'Infarto' },
      { id: 3, nome: 'Carlos Santos', leito: '103C', status: 'Estável', diagnostico: 'Fratura' },
    ]
  ...
  const handleStatusUpdate = (pacienteId: number, newStatus: string) => {
    setPacientes((prev) =>
      prev.map((p) => (p.id === pacienteId ? { ...p, status: newStatus } : p))
    )
